...
final class ConnectionsController extends WP_REST_Controller {
	...
	public function create_connection( $request ) {
		...
		try {
			...
			return new WP_Error( 'rootstuff_relationships_connection_error', $e->getMessage(), [ 'status' => 400 ] );
		}

		if ( false === $connection_id ) {
			return new WP_Error(
				'rootstuff_relationships_connection_failed',
	...
	public function delete_connection( $request ) {
		...
		try {
			...
			return new WP_Error( 'rootstuff_relationships_disconnect_error', $e->getMessage(), [ 'status' => 400 ] );
		}

		if ( ! $deleted ) {
			return new WP_Error(
				'rootstuff_relationships_connection_not_found',
	...
	public function sync_connections( $request ) {
		...
		try {
			...
			return new WP_Error( 'rootstuff_relationships_sync_error', $e->getMessage(), [ 'status' => 400 ] );
		}
