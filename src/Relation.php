...
final class Relation {
	...
	public static function connect( string $rel_type, int $from_id, int $to_id, array $args = [] ): int|false {
		...
		// phpcs:ignore WordPress.DB.DirectDatabaseQuery, PluginCheck.Security.DirectDB.UnescapedDBParameter.DirectQuery -- custom table; no WP API available.
		$result = $wpdb->insert(
			$table,
			[
				'rel_type'         => $rel_type,
		 ...
		 */
		do_action( 'rootstuff_relationships_connected', $rel_type, $from_id, $to_id, $connection_id );
	...
	public static function disconnect( string $rel_type, int $from_id, int $to_id ): bool {
		...
		$connection_id = (int) $wpdb->get_var( // phpcs:ignore WordPress.DB.DirectDatabaseQuery, PluginCheck.Security.DirectDB.UnescapedDBParameter
			$wpdb->prepare(
		...
		$deleted = $wpdb->delete( // phpcs:ignore WordPress.DB.DirectDatabaseQuery, PluginCheck.Security.DirectDB.UnescapedDBParameter
			$table,
			[
				'rel_type'       => $rel_type,
				'from_object_id' => $from_id,
		...
		if ( $deleted ) {
			...
			do_action( 'rootstuff_relationships_disconnected', $rel_type, $from_id, $to_id );
		}
	...
	private static function getIdsBoth( string $rel_type, int $object_id ): array {
		...
		$ids = $wpdb->get_col(
			$wpdb->prepare(
				"SELECT CASE WHEN from_object_id = %d THEN to_object_id ELSE from_object_id END AS related_id FROM {$table} WHERE rel_type = %s AND (from_object_id = %d OR to_object_id = %d) ORDER BY sort_order ASC, id ASC", // phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared
				$object_id,
				$rel_type,
				$object_id,
				$object_id
	...
	public static function disconnectAll( int $object_id ): int {
		...
		$deleted_from = $wpdb->delete( $table, [ 'from_object_id' => $object_id ], [ '%d' ] ); // phpcs:ignore WordPress.DB.DirectDatabaseQuery, PluginCheck.Security.DirectDB.UnescapedDBParameter
		$deleted_to   = $wpdb->delete( $table, [ 'to_object_id' => $object_id ], [ '%d' ] ); // phpcs:ignore WordPress.DB.DirectDatabaseQuery, PluginCheck.Security.DirectDB.UnescapedDBParameter

		$total = ( $deleted_from ?: 0 ) + ( $deleted_to ?: 0 );

		if ( $total > 0 ) {
			do_action( 'rootstuff_relationships_disconnected_all', $object_id, $total );
		}
	...
	private static function update_sort_order( string $rel_type, int $object_id, int $connected_id, string $direction, int $order ): void {
		...
		if ( 'from' === $direction ) {
			$wpdb->update( // phpcs:ignore WordPress.DB.DirectDatabaseQuery, PluginCheck.Security.DirectDB.UnescapedDBParameter
				$table,
				[ 'sort_order' => $order ],
				[
					'rel_type'       => $rel_type,
	...
	public static function getMeta( int $connection_id, string $key ): mixed {
		...
		$value = $wpdb->get_var( // phpcs:ignore WordPress.DB.DirectDatabaseQuery, PluginCheck.Security.DirectDB.UnescapedDBParameter
			$wpdb->prepare(
	...
	public static function setMeta( int $connection_id, string $key, mixed $value ): bool {
		...
		$existing = $wpdb->get_var( // phpcs:ignore WordPress.DB.DirectDatabaseQuery, PluginCheck.Security.DirectDB.UnescapedDBParameter
			$wpdb->prepare(
		...
		if ( $existing ) {
			$result = $wpdb->update( // phpcs:ignore WordPress.DB.DirectDatabaseQuery, PluginCheck.Security.DirectDB.UnescapedDBParameter
				$table,
		...
		$result = $wpdb->insert( // phpcs:ignore WordPress.DB.DirectDatabaseQuery, PluginCheck.Security.DirectDB.UnescapedDBParameter
			$table,
			[
	...
	public static function getConnectionId( string $rel_type, int $from_id, int $to_id ): ?int {
		...
		$id = $wpdb->get_var( // phpcs:ignore WordPress.DB.DirectDatabaseQuery, PluginCheck.Security.DirectDB.UnescapedDBParameter
			$wpdb->prepare(
				"SELECT id FROM {$table} WHERE rel_type = %s AND from_object_id = %d AND to_object_id = %d", // phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared
				$rel_type,
				$from_id,
