...
1. Activate **Rootstuff Relationships** in `wp-content/plugins/rootstuff-relationships/`.
2. In the active theme's `functions.php` (or a small mu-plugin) register a few demo relationships on `rootstuff_relationships_init` so the screens look populated, e.g.:
...
- **State:** Edit a post that has the Rootstuff Relationships sidebar open. Expand the panel so the search input is visible and 2-3 connected items are listed beneath it (with their drag handles and remove buttons). Crop tight around the sidebar.
