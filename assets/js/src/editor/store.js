...
const STORE_NAME = 'rootstuff-relationships/store';
	...
	actions: {
			...
			return async ( { dispatch } ) => {
				try {
					...
					const data = await apiFetch( {
						path: `/rootstuff-relationships/v1/connections/${ relType }/${ objectId }${ params }`,
					} );
					...
					console.error( 'Rootstuff Relationships: failed to fetch connections', error );
				}
			};
		},
			...
			return async ( { dispatch } ) => {
				...
				try {
					...
					const results = await apiFetch( {
						path: `/rootstuff-relationships/v1/search?${ params.toString() }`,
					} );
					dispatch( {
					...
					// eslint-disable-next-line no-console
					console.error( 'Rootstuff Relationships: search failed', error );
				} finally {
					dispatch( {
						type: 'SET_SEARCHING',
						relType,
		...
		saveConnections( relType, objectId, side ) {
			return async ( { dispatch, select } ) => {
				...
				try {
					await apiFetch( {
						path: `/rootstuff-relationships/v1/connections/${ relType }/${ objectId }/sync`,
						method: 'POST',
						data: {
							side,
					...
					// eslint-disable-next-line no-console
					console.error( 'Rootstuff Relationships: save failed', error );
				} finally {
					dispatch( {
						type: 'SET_SAVING',
						relType,
