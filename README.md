...
Register relationships on the `rootstuff_relationships_init` action using the `Schema` class:
...
Query related posts directly with `WP_Query` using the `rootstuff_relationships_related` parameter:
...
Endpoints are registered under `/wp-json/rootstuff-relationships/v1/`:
...
- `rootstuff_relationships_connected` — fires after a connection is created
- `rootstuff_relationships_disconnected` — fires after a connection is removed
- `rootstuff_relationships_disconnected_all` — fires after all connections for an object are removed

## Caching

All `getIds()` queries are cached using the WordPress object cache (`rootstuff-relationships` group). Caches are automatically invalidated on connect, disconnect, and sync operations.
